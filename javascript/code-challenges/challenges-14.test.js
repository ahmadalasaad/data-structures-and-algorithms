...
const screenForNames = (arr) => {
  // Solution code here...
  let newArr= arr.map((item) => item.match(/^(Mr|Mrs|Ms|Dr|)\..[A-Za-z]+$/g)).filter((item) => item).flat();
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.reduce((a, b) => a + (b.mass > 77 ? " - " + b.name : ""), "").slice(3);
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  if( typeof arr[0][property] === "string"){
