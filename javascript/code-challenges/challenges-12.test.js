...
const replaceZeros = (string) => {
  // Solution code here...
  var newStr = string.replace(/0/g, "zero");  
  return newStr
  ;}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
//  str= parseInt(pin).toString()
//   console.log(typeof str);
// }
// let arr=str.split('')
// return  arr.length===4
let numPin = /^(\d{4}|^\d)$/;
return numPin.test(pin);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const num = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
